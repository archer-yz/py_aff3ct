...
parser.add_argument("--doxy-xml-path", help="Path of the Doxygen XML files.",
                    default=command_path + "/lib/aff3ct/doc/build/doxygen/xml/")
parser.add_argument("--template-path", help="Path of the py_aff3ct *template* folder.",
                    default=command_path + "/template")

parser.add_argument("--include-module", nargs="+", help="List of aff3ct Modules to be wrapped. Example : --include-module Source Modem",  # noqa
                    default=['CRC', 'Channel', 'Modem', 'Source', 'Encoder', 'Decoder', 'Iterator', 'Sink'])
parser.add_argument("--exclude-module", nargs="+", help="List of aff3ct Modules to be excluded from the wrapper. (prioritary over --include-module). Example : --exclude-module Source Modem", default=['Modem_CPM', 'Modem_OOK', 'Encoder_RSC_generic_json_sys', 'Decoder_LDPC_bit_flipping', 'Decoder_chase_pyndiah', 'Decoder_turbo_product', 'Decoder_RSC_BCJR_seq_generic_std_json',  # noqa
                    'Reporter', 'Decoder_LDPC_BP_flooding_inter', 'Decoder_LDPC_BP_flooding_SPA', 'Decoder_LDPC_bit_flipping_hard', 'Decoder_polar_MK_SC_naive', 'Encoder_polar_MK', 'Decoder_polar_MK_SCL_naive', 'Decoder_polar_MK_SCL_naive_CA', 'Decoder_polar_MK_ASCL_naive_CA', 'Decoder_polar_MK_SCL_naive_CA_sys', 'Decoder_polar_MK_ASCL_naive_CA_sys', 'Decoder_polar_MK_SCL_naive_sys'])  # noqa
parser.add_argument("--include-tool",   nargs="+", help="List of aff3ct Tools to be wrapped. Example : --include-tool Constellation", default=[  # noqa
                    'Constellation', 'Pattern_polar_i', 'Noise', 'BCH_polynomial_generator', 'RS_polynomial_generator', 'Polar_code', 'Interleaver_core', 'dvbs2_values'])  # noqa
parser.add_argument("--exclude-tool",   nargs="+",
                    help="List of aff3ct Tools to be excluded from the wrapper. (prioritary over --include-tool). Example : --exclude-tool Constellation", default=[])  # noqa

args = parser.parse_args()

if args.clean:
    src_folder = Path(command_path + '/src')
    if args.verbose:
    ...
    for key2, value2 in doxygen.items():
        if key != key2 and "basecompoundref" in value2["compounddef"].keys():
            ...
            for ref in refs_list:
                if ref["#text"].split("<")[0].replace('aff3ct::module::', '').replace('aff3ct::tools::', '') == key.replace('aff3ct::module::', '').replace('aff3ct::tools::', ''):  # noqa
                    elmt = {"#text": key2}
...
tools_tree = {}
tools_classes_list = aff3ct_tools.recursive_build_classes_list(
    doxygen, args.include_tool, args.exclude_tool, "aff3ct::tools::", tools_tree)
...
module_tree = {}
module_classes_list = aff3ct_tools.recursive_build_classes_list(
    doxygen, args.include_module, args.exclude_module, "aff3ct::module::", module_tree)
